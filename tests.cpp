...
void save_load_test(string path, string source) {
	...
	saveCipher(public_key, path + "/ciphertext/publickey.txt");
	Plaintext secret_key = generator.secret_key();
	savePlain(secret_key, path + "/ciphertext/secretkey.txt");
	...
	*/
	FractionalEncoder encoder(parms.plain_modulus(), parms.poly_modulus(), 1, 8, 2);
	// Create encryptor, evaluator, decryptor
	Encryptor encryptor(parms, public_key);
	Evaluator evaluator(parms, evaluation_keys);
	Decryptor decryptor(parms, secret_key);

	Plaintext testPlain = encoder.encode(input[0][0]);
	savePlain(testPlain, path + "/ciphertext/plaintxt00.txt");

	// decode
	double dx = encoder.decode(testPlain);
	cout << dx << endl;

	cout << "encrypting x: " << endl;
	vector<vector<Ciphertext>> x = encrypt(input, encoder, encryptor); // encryption

	path.append("/ciphertext/");
	cout << "Saving ciphertext" << endl;
	ciphertextSave2D(x, path);
	cout << "Saving keys" << endl;
	saveKeys(public_key, secret_key, evaluation_keys, path);
	cout << "Saving parms" << endl;
	saveParms(parms, path);
	
	//save cipher text into txt file
	saveCipher(x[0][0], path + "ciphertxt00.txt");
	...
	for (int i = 0; i < x.size(); i++) {
		for (int j = 0; j < x[0].size(); j++) {
			Plaintext tr = decryptor.decrypt(x[i][j]);
			double tr_double = encoder.decode(tr);
			Plaintext pr = decryptor.decrypt(res[i][j]);
			double pr_double = encoder.decode(pr);
			if (tr_double == pr_double)
				printf("[%d, %d] matched", i, j);
			else {
				printf("[%d, %d] not matched", i, j);
				break;
			}
		}
	}
	saveCipher(res[0][0], path + "test2.txt");

}
void average_pooling_test(string path) {
	path.append("/ciphertext/");

	cout << "Loading parms ..." << endl;
	EncryptionParameters parms;
	loadParms(parms, path);
	parms.validate(); // parms.validate() is the end of parms creation

	cout << "Loading keys ..." << endl;
	Ciphertext public_key = Ciphertext::Ciphertext(BigPolyArray());
	Plaintext secret_key;
	EvaluationKeys evaluation_keys;
	loadKeys(public_key, secret_key, evaluation_keys, path);
	...
	*/
	...
	int rowSize = 2;
	int colSize = 16;
	vector<vector<Ciphertext>> loaded;
	ciphertextLoad2D(loaded, path);

	//averaging
	vector<vector<Ciphertext>> res;
	res = averagePooling2D(loaded, 2, 2, encoder, evaluator);

	// decode
	vector<vector<double>> output;
	for (int i = 0; i < res.size(); i++) {
		vector<double> output1D;
		for (int j = 0; j < res[0].size(); j++) {
			Plaintext pr = decryptor.decrypt(res[i][j]);
			double pr_double = encoder.decode(pr);
			output1D.emplace_back(pr_double);
		}
		output.emplace_back(output1D);
	}
...
void monitor_test(string path, string source) {
	...
	*/
	...
	cout << "original: " << input[0][0] << endl;
	Plaintext testPlain = encoder.encode(input[0][0]);
	savePlain(testPlain, path + "/ciphertext/plaintxt00.txt");
	cout << "Plaintext: " << returnPlain(testPlain) << endl;
	// decode
	double test = encoder.decode(testPlain);
	cout << "Decode: " << test << endl;

	cout << "encrypting x[0][0]: " << endl;
	Ciphertext x = Ciphertext(BigPolyArray());
	encryptor.encrypt(testPlain, x); // encryption
	cout << "Noise budget in encryption of x[0][0]" << ": " << decryptor.invariant_noise_budget(x) << " bits" << endl;
	saveCipher(x, path + "/ciphertext/ciphertxt00.txt");

	cout << "decrypting x[0][0]: " << endl;
	Plaintext dx = decryptor.decrypt(x);
	savePlain(dx, path + "/ciphertext/dec_plaintxt00.txt");
	cout << "Decrypt Plaintext: " << returnPlain(dx) << endl;

	// decode again
	double test2 = encoder.decode(dx);
	cout << "Decode again: " << test2 << endl;

	// multiply
	cout << "Calculate square: " << endl;
	Ciphertext x2 = Ciphertext(BigPolyArray());
	x2 = evaluator.square(x);
	saveCipher(x2, path + "/ciphertext/square_ciphertxt00.txt");
	cout << "Noise budget in encryption of x^2" << ": " << decryptor.invariant_noise_budget(x2) << " bits" << endl;
	Plaintext dx2 = decryptor.decrypt(x2);
	savePlain(dx2, path + "/ciphertext/square_plaintxt00.txt");
	cout << "Decrypt square plaintext: " << returnPlain(dx2) << endl;
	double testsquare = encoder.decode(dx2);
	cout << "Square: " << testsquare << endl;
	cout << "True Square: " << test2 * test2 << endl;

	cout << "Calculate square square: " << endl;
	Ciphertext x4 = Ciphertext(BigPolyArray());
	x4 = evaluator.square(x2);
	saveCipher(x4, path + "/ciphertext/square_square_ciphertxt00.txt");
	cout << "Noise budget in encryption of x^4" << ": " << decryptor.invariant_noise_budget(x4) << " bits" << endl;
	Plaintext dx4 = decryptor.decrypt(x4);
	savePlain(dx4, path + "/ciphertext/square_square_plaintxt00.txt");
	cout << "Decrypt square sqaure plaintext: " << returnPlain(dx4) << endl;
	double testsquaresquare = encoder.decode(dx4);
	cout << "Square sqaure: " << testsquaresquare << endl;
	cout << "True Square sqaure: " << test2 * test2 * test2 * test2 << endl;

	cout << "Calculate square square square: " << endl;
	Ciphertext x8 = Ciphertext(BigPolyArray());
	x8 = evaluator.square(x4);
	saveCipher(x8, path + "/ciphertext/square_square_square_ciphertxt00.txt");
	cout << "Noise budget in encryption of x^8" << ": " << decryptor.invariant_noise_budget(x8) << " bits" << endl;
	Plaintext dx8 = decryptor.decrypt(x8);
	savePlain(dx8, path + "/ciphertext/square_square_square_plaintxt00.txt");
	cout << "Decrypt square sqaure plaintext: " << returnPlain(dx8) << endl;
	double testsquaresquaresquare = encoder.decode(dx8);
	cout << "Square sqaure square: " << testsquaresquaresquare << endl;
	cout << "True Square sqaure square: " << test2 * test2 * test2 * test2 * test2 * test2 * test2 * test2 << endl;
	// load the pre-existing files
	/*path.append("/ciphertext/");
...
void CNN_test_slim() {
	...
	parms.validate(); // parms.validate() is the end of parms creation
					  ...
					  // Generate keys.
	cout << "Slim version _v1.0 _ Generating keys ..." << endl;
	KeyGenerator generator(parms);
	generator.generate(4); // int parameter is the evaluation key count 
	cout << "... key generation complete" << endl;
	Ciphertext public_key = generator.public_key();
...
void example_linear_regression()
{
	...
	// In this example we demonstrate computing linear regression for diabetes, label is consistive: a quantitative measure of disease progression;

	// input X has 10 features ([-0.07816532,  0.05068012,  0.07786339,  0.05285819,  0.07823631,	0.0644473, 0.02655027, -0.00259226, 0.04067226, -0.00936191])

	// below is X to be predict:
	const vector<double> rational_numbers{ -0.07816532,  0.05068012,  0.07786339,  0.05285819,  0.07823631,	0.0644473, 0.02655027, -0.00259226, 0.04067226, -0.00936191 };

	// The weights are fit through python->sklearn:
	const vector<double> coefficients{ 0.3034995490664121, -237.63931533353392, 510.5306054362245, 327.7369804093466, -814.1317093725389, 492.81458798373245, 102.84845219168025, 184.60648905984064, 743.5196167505419, 76.095172216624 };
	...
	/*
	We will need a fractional encoder for dealing with the rational numbers. Here we reserve
	32 coefficients of the polynomial for the integral part (low-degree terms) and expand the
	fractional part to 64 terms of precision (base 3) (high-degree terms).
	*/
	//FractionalEncoder encoder(parms.plain_modulus(), parms.poly_modulus(), 64, 32, 3);
	FractionalEncoder encoder(parms.plain_modulus(), parms.poly_modulus(), 32, 64, 3);
	...
	// We also need to encode bias. We will add_plain the result by this to got bias.
	//const double bias = 152.764306916;
	Plaintext bias = encoder.encode(152.764306916);
	...
	// Finally we add bias to obtain the result.
	cout << "adding bias ... ";
	Ciphertext encrypted_result = evaluator.add_plain(encrypted_dot_product, bias);
	cout << "done." << endl;
...
void example_basics()
{
	...
	*/
	...
	//save encrypted into binary file from Ciphertext
	ofstream myfile;
	myfile.open("firstTrial.txt", std::ofstream::out | std::ofstream::binary);
	// save encrypted
	encrypted1.save(myfile);
	myfile.close();

	// generate an empty Ciphertext to load the saved binary file
	Ciphertext encryptedload = Ciphertext::Ciphertext(BigPolyArray());
	ifstream myfileLoad;
	myfileLoad.open("firstTrial.txt", std::ifstream::in | std::ifstream::binary);
	encryptedload.load(myfileLoad);// load encrypted1 that was encrypted saved previously
	myfileLoad.close();
	...
	Ciphertext encryptednegated1 = evaluator.negate(encryptedload);
...
void example_relinearization()
{
	...
	*/
	...
	/*
	Example 3: I want to try to use Fraction encoder to get plainText then do the same thing for relinearization
	*/
	example_relinearization_part3();
}
...
{
	print_example_banner("Example: relinearization with Fraction encoder");

	// In this example we demonstrate computing a weighted average of 10 rational numbers.

	// The 10 rational numbers we use are:
	const vector<double> rational_numbers{ 3.00000001 };
	// The 10 weights are:
	const vector<double> coefficients{ 0.10000001 };
	...
	*/
	...
	// Next we encrypting coefficients ...
	cout << "Encrypting coefficients ... ";
	vector<Ciphertext> encrypted_coefficients;
	for (int i = 0; i < coefficients.size(); i++)
	{
		Plaintext encoded_coefficients = encoder.encode(coefficients[i]);
		encrypted_coefficients.emplace_back(encryptor.encrypt(encoded_coefficients));
		cout << to_string(coefficients[i]).substr(0, 6) << ((i < 9) ? ", " : ".\n");
	}

	// We also need to encode 0.1. We will multiply the result by this to perform division by size.
	double vectorSize = 1 / rational_numbers.size();
	Plaintext div_by_size = encoder.encode(vectorSize);

	// Now compute all the products of the encrypted rational numbers with the plaintext coefficients
	cout << "Computing products ... ";
	vector<Ciphertext> encrypted_products;
	for (int i = 0; i < encrypted_rationals.size(); i++)
	{
		/*
		We use Evaluator::multiply_plain(...) instead of Evaluator::multiply(...) (which would
		require also the coefficient to be encrypted). This has much better noise growth
		behavior than multiplying two encrypted numbers does.
		*/
		Ciphertext enc_product = evaluator.multiply(encrypted_rationals[i], encrypted_coefficients[i]);
		encrypted_products.emplace_back(enc_product);
	}
	cout << "computing ai*xi products is done." << endl;

	// Now we add together these products. The most convenient way to do that is
	// to use the function Evaluator::add_many(...).
	cout << "Add up all multiplied ciphertexts ... ";
	Ciphertext encrypted_dot_product = evaluator.add_many(encrypted_products);
	cout << "sigma(ai*xi) is done." << endl;

	// Finally we divide by size to obtain the result.
	cout << "Divide by size ... ";
	Ciphertext encrypted_result = evaluator.multiply_plain(encrypted_dot_product, div_by_size);
	cout << "done." << endl;
	// (a0*x0)^2 => cipher size = 5
	Ciphertext squar_encrypted = evaluator.square(encrypted_result);

	// Print size and noise budget of result. 
	cout << "Size of weightedSum without relinearization: " << encrypted_result.size() << endl;
	cout << "Noise budget in weightedSum without relinearization: " << decryptor.invariant_noise_budget(encrypted_result) << " bits" << endl;
	cout << "Size of (weightedSum) ^ 2 without relinearization: " << squar_encrypted.size() << endl;
	cout << "Noise budget in weightedSum without relinearization: " << decryptor.invariant_noise_budget(squar_encrypted) << " bits" << endl;

	// Decrypt the result without relinear
	cout << "Decrypting result without relinearization ... ";
	Plaintext plain_result = decryptor.decrypt(encrypted_result);
	Plaintext plain_result_square = decryptor.decrypt(squar_encrypted);
	cout << "done." << endl;

	// Print the result
	double result = encoder.decode(plain_result);
	double resultSquare = encoder.decode(plain_result_square);
	cout << "Weighted average of weightedSum without relinear: " << resultSquare << endl;

	// now lets add relinearization
	print_example_banner("with relinearization");
	Ciphertext enc_relin_result = evaluator.relinearize(encrypted_result);
	Ciphertext enc_relin_resultSquare = evaluator.relinearize(squar_encrypted);
	// Print size and noise budget of result after relinearization. 
	cout << "Size of weightedSum with relinearization: " << enc_relin_result.size() << endl;
	cout << "Noise budget in weightedSum with relinearization: " << decryptor.invariant_noise_budget(enc_relin_result) << " bits" << endl;
	cout << "Size of weightedSum with relinearization: " << enc_relin_resultSquare.size() << endl;
	cout << "Noise budget in weightedSum with relinearization: " << decryptor.invariant_noise_budget(enc_relin_resultSquare) << " bits" << endl;
	// Decrypt result with relinear
	cout << "Decrypting result with relinearization ... ";
	Plaintext plain_relin_result = decryptor.decrypt(enc_relin_result);
	Plaintext plain_relin_resultSquare = decryptor.decrypt(enc_relin_resultSquare);
	double result_relin = encoder.decode(plain_relin_result);
	double square_result_relin = encoder.decode(plain_relin_resultSquare);
	cout << "Weighted average of weightedSum with relinear: " << result_relin << endl;
	cout << "Weighted average of weightedSum with relinear: " << square_result_relin << endl;
